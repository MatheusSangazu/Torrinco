...
import { Phone, Lock, ArrowRight, Loader2, Eye, EyeOff, CheckCircle } from 'lucide-react';
...
export function FirstAccess() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.error || 'Falha ao definir senha. Verifique se o número está correto ou se já possui senha.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400">Defina sua senha para começar</p>
      </div>
