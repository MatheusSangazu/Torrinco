    ...
    <div className="space-y-5 pb-28 lg:space-y-6 lg:pb-24">
      ...
      {!current ? (
        ...
        <>
          ...
          {current.document_type === 'card_statement' && <Notice tone="info" icon={<Clock3 size={18} />} title="Como tratamos uma fatura aberta">As compras serão registradas nas datas originais e entrarão no ciclo do cartão escolhido. O vencimento é calculado pelos dias de fechamento e vencimento cadastrados no cartão. Se você importar depois a fatura atualizada, os lançamentos já existentes serão sinalizados como possíveis duplicidades.</Notice>}
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm sm:p-4 dark:border-slate-700 dark:bg-slate-800">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="grid gap-2 sm:grid-cols-[minmax(190px,1fr)_auto]">
                <CustomSelect value={bulkCategory} onChange={value => setBulkCategory(String(value))} placeholder="Categoria em massa" options={categories.map(item => ({ value: String(item.id), label: item.name }))}/>
          ...
          {!shown.length ? <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-800"><SearchX className="mx-auto text-gray-400"/><p className="mt-2 font-medium text-gray-700 dark:text-slate-200">Nenhum item corresponde ao filtro.</p></div> : <>
            <ReviewTable items={shown} entities={entities} categories={categories} patch={patchItem} />
            <div className="space-y-3 lg:hidden">{shown.map(item => <ReviewMobileItem key={item.id} item={item} entities={entities} categories={categories} patch={data => patchItem(item, data)} />)}</div>
          </>}
...
function ImportHeader({ value, entities, update }: { value: FinancialImport; entities: Option[]; update: (data: object) => Promise<void> }) { return <section className="relative z-20 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-5 dark:border-slate-700 dark:bg-slate-800"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex items-center gap-3"><span className="rounded-xl bg-torrinco-50 p-2 text-torrinco-600 dark:bg-torrinco-950/30"><FileSpreadsheet size={22}/></span><div className="min-w-0"><h2 className="truncate font-bold text-gray-800 dark:text-white">{value.file_name}</h2><div className="mt-1 flex flex-wrap items-center gap-2"><span className={clsx('rounded-full px-2.5 py-1 text-xs font-semibold', statusStyle(value.status))}>{labels[value.status] || value.status}</span><span className="text-xs text-gray-500">{value.items.length} itens encontrados</span></div></div></div></div><div className="w-full lg:max-w-sm"><CustomSelect label="Destino da importação" value={value.target_entity_id || ''} disabled={value.status !== 'review'} onChange={selected => void update({ target_entity_id: selected === '' ? null : Number(selected) })} placeholder="Selecione uma conta ou cartão" options={entities.map(item => ({ value: item.id, label: item.name }))}/></div></div></section>; }

function Notice({ icon, title, children, tone }: { icon: React.ReactNode; title: string; children: React.ReactNode; tone: 'warning' | 'danger' | 'info' }) { const tones = { warning: 'border-amber-200 bg-amber-50/80 text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200', danger: 'border-red-200 bg-red-50 text-red-900 dark:border-red-800 dark:bg-red-950/30 dark:text-red-200', info: 'border-blue-200 bg-blue-50/80 text-blue-900 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-200' }; return <div role="alert" className={clsx('flex gap-3 rounded-2xl border p-4', tones[tone])}><span className="mt-0.5 shrink-0">{icon}</span><div><p className="text-sm font-bold">{title}</p><div className="mt-0.5 text-sm opacity-90">{children}</div></div></div>; }
...
function Selector({ value, items, onChange, label = 'Sem vínculo' }: { value?: number | null; items: Option[]; onChange: (value: number | null) => void; label?: string }) { return <CustomSelect value={value || ''} onChange={selected => onChange(selected === '' ? null : Number(selected))} placeholder={label} options={items.map(item => ({ value: item.id, label: item.name }))}/>; }

function EnumSelector({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string }> }) { return <CustomSelect value={value} onChange={selected => onChange(String(selected))} options={options}/>; }
...
function ReviewTable({ items, entities, categories, patch }: { items: ImportItem[]; entities: Option[]; categories: Option[]; patch: (item: ImportItem, data: object) => void }) {
  return <div className="hidden overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm lg:block dark:border-slate-700 dark:bg-slate-800">
    <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm">
      <thead className="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:bg-slate-900/50 dark:text-slate-400"><tr>{['Incluir', 'Data', 'Descrição original', 'Descrição salva', 'Valor', 'Tipo', 'Categoria', 'Conta/cartão', 'Situação'].map(label => <th className="px-3 py-3" key={label}>{label}</th>)}</tr></thead>
      <tbody className="divide-y divide-gray-100 dark:divide-slate-700">{items.map(item => {
        const hasAlert = Boolean(item.duplicate_reason || item.exclusion_reason || item.requires_review);
        return <tr key={item.id} className={clsx('align-top transition', !item.included && 'bg-gray-50/70 opacity-60 dark:bg-slate-900/30')}>
          <td className="px-3 py-3"><input aria-label="Incluir lançamento" className="h-4 w-4 accent-torrinco-600" type="checkbox" checked={item.included} onChange={event => patch(item, { included: event.target.checked })}/></td>
          <td className="w-36 px-2 py-3"><input className={fieldClass} type="date" defaultValue={item.transaction_date.slice(0, 10)} onBlur={event => patch(item, { transaction_date: event.target.value })}/></td>
          <td className="max-w-60 px-3 py-3 text-gray-600 dark:text-slate-300" title={item.original_excerpt}><p>{item.original_description}</p>{hasAlert && <div className="mt-2"><ItemAlert item={item}/></div>}</td>
          <td className="w-52 px-2 py-3"><input className={fieldClass} defaultValue={item.description} onBlur={event => event.target.value !== item.description && patch(item, { description: event.target.value })}/></td>
          <td className="w-32 px-2 py-3"><input className={fieldClass} type="number" step=".01" defaultValue={Number(item.amount)} onBlur={event => patch(item, { amount: Number(event.target.value) })}/></td>
          <td className="w-36 px-2 py-3"><EnumSelector value={item.type} onChange={value => patch(item, { type: value })} options={[{ value: 'expense', label: 'Despesa' }, { value: 'income', label: 'Receita' }]}/></td>
          <td className="w-44 px-2 py-3"><Selector value={item.category_id} items={categories} onChange={value => patch(item, { category_id: value })}/></td>
          <td className="w-44 px-2 py-3"><Selector value={item.entity_id} items={entities} onChange={value => patch(item, { entity_id: value })}/></td>
          <td className="w-36 px-2 py-3"><EnumSelector value={item.transaction_status} onChange={value => patch(item, { transaction_status: value })} options={[{ value: 'paid', label: 'Pago' }, { value: 'pending', label: 'Pendente' }]}/></td>
        </tr>;
      })}</tbody>
    </table></div>
  </div>;
...
function ReviewMobileItem({ item, entities, categories, patch }: { item: ImportItem; entities: Option[]; categories: Option[]; patch: (data: object) => void }) {
  const [expanded, setExpanded] = useState(Boolean(item.requires_review || item.duplicate_kind));
  const hasAlert = Boolean(item.duplicate_reason || item.exclusion_reason || item.requires_review);
  return <article className={clsx('relative overflow-visible rounded-2xl border bg-white shadow-sm dark:bg-slate-800', item.included ? 'border-gray-100 dark:border-slate-700' : 'border-gray-200 opacity-65 dark:border-slate-700')}>
    <div className="p-4"><div className="flex items-start gap-3">
      <input aria-label="Incluir lançamento" className="mt-1 h-5 w-5 shrink-0 accent-torrinco-600" type="checkbox" checked={item.included} onChange={event => patch({ included: event.target.checked })}/>
      <button className="min-w-0 flex-1 text-left" onClick={() => setExpanded(value => !value)}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold text-gray-800 dark:text-white">{item.description}</p><p className="mt-0.5 text-xs text-gray-500">{new Date(`${item.transaction_date.slice(0, 10)}T12:00:00`).toLocaleDateString('pt-BR')} · {item.type === 'expense' ? 'Despesa' : 'Receita'}</p></div><p className={clsx('shrink-0 font-bold', item.type === 'expense' ? 'text-red-600' : 'text-emerald-600')}>{money(item.amount)}</p></div><div className="mt-3 flex items-center justify-between gap-2">{hasAlert ? <ItemAlert item={item}/> : <span/>}<ChevronRight size={18} className={clsx('shrink-0 text-gray-400 transition', expanded && 'rotate-90')}/></div></button>
    </div></div>
    {expanded && <div className="space-y-3 border-t border-gray-100 bg-gray-50/60 p-4 dark:border-slate-700 dark:bg-slate-900/30">
      <Field label="Descrição original"><p className="rounded-xl border border-gray-200 bg-white p-3 text-sm text-gray-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300">{item.original_description}</p></Field>
      <Field label="Descrição salva"><input className={fieldClass} defaultValue={item.description} onBlur={event => event.target.value !== item.description && patch({ description: event.target.value })}/></Field>
      <div className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2"><Field label="Data"><input className={fieldClass} type="date" defaultValue={item.transaction_date.slice(0, 10)} onBlur={event => patch({ transaction_date: event.target.value })}/></Field><Field label="Valor"><input className={fieldClass} type="number" step=".01" defaultValue={Number(item.amount)} onBlur={event => patch({ amount: Number(event.target.value) })}/></Field><Field label="Tipo"><EnumSelector value={item.type} onChange={value => patch({ type: value })} options={[{ value: 'expense', label: 'Despesa' }, { value: 'income', label: 'Receita' }]}/></Field><Field label="Situação"><EnumSelector value={item.transaction_status} onChange={value => patch({ transaction_status: value })} options={[{ value: 'paid', label: 'Pago' }, { value: 'pending', label: 'Pendente' }]}/></Field></div>
      <Field label="Categoria"><Selector value={item.category_id} items={categories} onChange={value => patch({ category_id: value })}/></Field>
      <Field label="Conta ou cartão"><Selector value={item.entity_id} items={entities} onChange={value => patch({ entity_id: value })}/></Field>
    </div>}
  </article>;
}
