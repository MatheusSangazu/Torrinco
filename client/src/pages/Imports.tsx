          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm sm:p-4 dark:border-slate-700 dark:bg-slate-800">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="grid gap-2 sm:grid-cols-[minmax(190px,1fr)_auto]">
                <Select className="text-sm" value={bulkCategory} onChange={event => setBulkCategory(event.target.value)}><option value="">Categoria em massa</option>{categories.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</Select>
...
function Selector({ value, items, onChange, label = 'Sem vínculo' }: { value?: number | null; items: Option[]; onChange: (value: number | null) => void; label?: string }) { return <Select className="text-sm" value={value || ''} onChange={event => onChange(event.target.value ? Number(event.target.value) : null)}><option value="">{label}</option>{items.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>; }
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm">
      ...
      <tbody className="divide-y divide-gray-100 dark:divide-slate-700">{items.map(item => {
        ...
        return <tr key={item.id} className={clsx('align-top transition', !item.included && 'bg-gray-50/70 opacity-60 dark:bg-slate-900/30')}>
          ...
          <td className="w-32 px-2 py-3"><Select className="text-sm" value={item.type} onChange={event => patch(item, { type: event.target.value })}><option value="expense">Despesa</option><option value="income">Receita</option></Select></td>
          ...
          <td className="w-32 px-2 py-3"><Select className="text-sm" value={item.transaction_status} onChange={event => patch(item, { transaction_status: event.target.value })}><option value="paid">Pago</option><option value="pending">Pendente</option></Select></td>
        </tr>;
      })}</tbody>
    </table></div>
  </div>;
...
function ReviewMobileItem({ item, entities, categories, patch }: { item: ImportItem; entities: Option[]; categories: Option[]; patch: (data: object) => void }) {
  ...
  const hasAlert = Boolean(item.duplicate_reason || item.exclusion_reason || item.requires_review);
  return <article className={clsx('overflow-hidden rounded-2xl border bg-white shadow-sm dark:bg-slate-800', item.included ? 'border-gray-100 dark:border-slate-700' : 'border-gray-200 opacity-65 dark:border-slate-700')}>
    ...
    {expanded && <div className="space-y-3 border-t border-gray-100 bg-gray-50/60 p-4 dark:border-slate-700 dark:bg-slate-900/30">
      ...
      <div className="grid grid-cols-1 gap-3 min-[380px]:grid-cols-2"><Field label="Data"><input className={fieldClass} type="date" defaultValue={item.transaction_date.slice(0, 10)} onBlur={event => patch({ transaction_date: event.target.value })}/></Field><Field label="Valor"><input className={fieldClass} type="number" step=".01" defaultValue={Number(item.amount)} onBlur={event => patch({ amount: Number(event.target.value) })}/></Field><Field label="Tipo"><Select className="text-sm" value={item.type} onChange={event => patch({ type: event.target.value })}><option value="expense">Despesa</option><option value="income">Receita</option></Select></Field><Field label="Situação"><Select className="text-sm" value={item.transaction_status} onChange={event => patch({ transaction_status: event.target.value })}><option value="paid">Pago</option><option value="pending">Pendente</option></Select></Field></div>
