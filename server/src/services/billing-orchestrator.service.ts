...
export class BillingOrchestratorService {
  ...
  private static async transition(order:any,status:'active'|'past_due'|'cancelled'|'suspended',reason:string,event:InternalBillingEvent,provider:string){const account=await prisma.accounts.findUnique({where:{id:order.account_id}});if(!account)return this.review(order.id,'ACCOUNT_NOT_FOUND');await prisma.$transaction([prisma.accounts.update({where:{id:account.id},data:{status}}),prisma.subscription_history.create({data:{account_id:account.id,plan_id:order.plan_id,previous_status:account.status,new_status:status,reason,metadata:{provider,eventId:event.providerEventId}}}),prisma.billing_subscriptions.updateMany({where:{order_id:order.id,provider},data:{status:status==='suspended'?'cancelled':status,last_event_at:event.occurredAt}})]);invalidateAccountStatusCache(account.id);return{processed:true,accountId:account.id}}
}
