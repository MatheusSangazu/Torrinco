...
function classifyItem(description: string, signedAmount: number): Pick<ParsedImportItem, 'kind' | 'type' | 'included' | 'exclusionReason' | 'requiresReview'> {
  const d = normalizeDescription(description);
  if (/pagamento (recebido|da fatura)|payment received/.test(d)) return { kind: 'bill_payment', type: 'income', included: false, exclusionReason: 'Pagamento de fatura não é uma nova transação.', requiresReview: false };
  ...
  if (/credito|crédito/.test(description.toLowerCase()) || signedAmount < 0) return { kind: 'credit', type: 'income', included: true, requiresReview: false };
...
const aliases = {
  date: ['data', 'date', 'transaction date', 'dt'], description: ['descricao', 'descrição', 'description', 'historico', 'histórico', 'memo', 'lancamento', 'lançamento'],
...
function findValue(row: Record<string, unknown>, names: string[]): unknown {
  const entry = Object.entries(row).find(([key]) => names.includes(normalizeDescription(key)));
  return entry?.[1];
}
...
export function parseSpreadsheetRows(buffer: Buffer, fileName: string): ParsedFinancialDocument {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const classified = classifyItem(description, signed);
    items.push({ rowIndex: index + 1, date, originalDescription: description, description: description.slice(0, 255), excerpt: JSON.stringify(row).slice(0, 1000), amount: Math.abs(signed), confidence: 0.95, ...classified });
  }
  if (!items.length) throw new ImportFileError('NO_ITEMS_FOUND', 'Nenhum lançamento válido foi identificado na planilha. Confira os cabeçalhos de data, descrição e valor.');
  return { documentType: 'spreadsheet', items };
}
...
export async function parsePdf(buffer: Buffer): Promise<ParsedFinancialDocument> {
  ...
  return { documentType, cardLastFour: lastFour, documentTotal: extractInvoiceTotal(text), items };
}
