...
export class RefreshTokenService {
  static async createRefreshToken(userId: number, accountId: number, userRole: string): Promise<string> {
    try {
  ...
  static async verifyRefreshToken(token: string): Promise<{ valid: boolean; payload?: JwtPayload; error?: string }> {
    try {
  ...
  static async rotateRefreshToken(oldToken: string): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    const cleanPayload: JwtPayload = { userId: payload.userId, accountId: payload.accountId, userRole: payload.userRole };
