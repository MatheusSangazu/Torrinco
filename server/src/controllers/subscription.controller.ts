...
export class SubscriptionController {
  ...
  static async cancel(req: JwtRequest, res: Response, next: NextFunction) {
    try { if (req.userRole !== 'admin') return res.status(403).json({ error: 'Admin requerido' }); res.json({ account: await changeSubscriptionStatus(req.accountId!, 'cancelled', 'cancelled_by_account_admin') }); } catch (e) { next(e); }
  }
  static async reactivate(req: JwtRequest, res: Response, next: NextFunction) {
    try {
      if (req.userRole !== 'admin') return res.status(403).json({ error: 'Admin requerido' });
