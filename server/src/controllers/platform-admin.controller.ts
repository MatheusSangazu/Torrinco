...
const maskEmail=(email:string|null)=>email?.replace(/^(.)(.*)(@.*)$/,'$1***$3')??null;
export class PlatformAdminController {
  ...
  static async plans(_req:PlatformRequest,res:Response,next:NextFunction){try{res.json({plans:await PlatformAdminService.plans()})}catch(e){next(e)}}
  static async accounts(req:PlatformRequest,res:Response,next:NextFunction){try{res.json(await PlatformAdminService.listAccounts(getValidatedQuery(req)))}catch(e){next(e)}}
  static async account(req:PlatformRequest,res:Response,next:NextFunction){try{const {id}=getValidatedParams<{id:number}>(req);const account=await PlatformAdminService.account(id);if(!account)return res.status(404).json({error:'Conta não encontrada'});res.json({account:{...account,users:account.users.map(u=>({...u,phone_number:maskPhone(u.phone_number),email:maskEmail(u.email)}))}})}catch(e){next(e)}}
  ...
  static async history(req:PlatformRequest,res:Response,next:NextFunction){try{res.json(await PlatformAdminService.history(getValidatedQuery(req)))}catch(e){next(e)}}
}
