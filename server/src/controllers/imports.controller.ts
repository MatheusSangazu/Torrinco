...
const optionalId = z.preprocess(v => v === null || v === '' ? undefined : v, z.coerce.number().int().positive().optional());
...
export class ImportsController {
  static async upload(req: JwtRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await FinancialImportService.upload({ ...scope(req), buffer: req.file.buffer, fileName: req.file.originalname, mimeType: req.file.mimetype, fileSize: req.file.size });
      res.status(201).json(result);
    } catch (error) { next(error); }
  }
  static async list(req: JwtRequest, res: Response, next: NextFunction) { try { res.json({ imports: await FinancialImportService.list(scope(req)) }); } catch (e) { next(e); } }
  ...
  static async confirm(req: JwtRequest, res: Response, next: NextFunction) { try { const body = z.object({ allow_difference: z.boolean().optional(), difference_reason: z.string().trim().max(500).optional() }).parse(req.body); res.json(await FinancialImportService.confirm(scope(req), id(req.params.id), body)); } catch (e) { next(e); } }
