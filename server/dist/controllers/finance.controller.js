...
export class FinanceController {
     ...
     */
    static projectRecurringTransactions(recurringTransactions, start, end, transactionsForCheck) {
        ...
        for (const rt of recurringTransactions) {
            ...
            while (currentDate <= end) {
                const existingTransaction = transactionsForCheck.find(t => t.is_recurring &&
                    t.type === rt.type &&
    ...
    static async list(req, res, next) {
        try {
            ...
            const end = end_date ? new Date(end_date) : new Date(2100, 0, 1);
            ...
            const projectedRecurring = FinanceController.projectRecurringTransactions(recurringTransactions, start, end, transactionsForCheck);
            // Combinar transações reais com projeções e ordenar
            const allTransactions = [...transactions, ...projectedRecurring].sort((a, b) => new Date(b.transaction_date).getTime() - new Date(a.transaction_date).getTime());
    ...
    static async getForecast(req, res, next) {
        try {
            ...
            const creditCardNextBillTransactions = [];
    ...
    static async getCardBill(req, res, next) {
        try {
    ...
    static async getCardNextBill(req, res, next) {
        try {
            const { cardId } = req.params;
            const userId = req.userId;
            const card = await prisma.financial_entities.findFirst({
