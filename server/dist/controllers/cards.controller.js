...
function calculateBillPeriod(closingDay, dueDay, referenceDate = new Date()) {
...
function calculateHistoricalBillPeriod(closingDay, dueDay, year, month) {
...
export class CardsController {
    static async list(req, res, next) {
        try {
            ...
            const targetMonth = month ? parseInt(month) : today.getMonth();
    ...
    static async getBillHistory(req, res, next) {
        try {
            const { id } = req.params;
            ...
            const closingDay = card.closing_day || 1;
            const dueDay = card.due_day || 10;
            const historyLength = Math.min(parseInt(months), 24);
            const bills = [];
            const today = new Date();
